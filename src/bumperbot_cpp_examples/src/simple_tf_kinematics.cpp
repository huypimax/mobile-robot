    ...
    , rotation_counter_(0)
{
    ...
    static_transform_stamped_.transform.translation.x = 0.0;
    static_transform_stamped_.transform.translation.x = 0.3;
...
{
    dynamic_transform_stmaped_.header.stamp = get_clock()->now();
    dynamic_transform_stmaped_.header.frame_id = "odom";
    dynamic_transform_stmaped_.child_frame_id = "bumperbot_base";
    dynamic_transform_stmaped_.transform.translation.x = last_x_ + x_increment_;
    dynamic_transform_stmaped_.transform.translation.y = 0.0;
    dynamic_transform_stmaped_.transform.translation.z = 0.0;
    tf2::Quaternion q;
    q = last_orientation_ * orientation_increment_;
    q.normalize();
    dynamic_transform_stmaped_.transform.rotation.x = q.x();
    dynamic_transform_stmaped_.transform.rotation.y = q.y();
    dynamic_transform_stmaped_.transform.rotation.z = q.z();
    dynamic_transform_stmaped_.transform.rotation.w = q.w();

    dynamic_tf_broadcaster_->sendTransform(dynamic_transform_stmaped_);

    last_x_ = dynamic_transform_stmaped_.transform.translation.x;
    rotation_counter_++;
    last_orientation_ = q;
                                              ...
                                              const std::shared_ptr<bumperbot_msgs::srv::GetTransform::Response> res)
{
    ...
    catch(tf2::TransformException &ex)
    {
        RCLCPP_ERROR_STREAM(get_logger(), "An error occurred while transforming from: " 
                            << req->frame_id << " to " << req->child_frame_id << ": " << ex.what());
        res->success = false;
        return true;
    }
